...
void updatestatus() {
  ...
  /*Serial.println(bumpsensor.sensor_read[0]);
  Serial.println(bumpsensor.sensor_read[1]);
  Serial.println(bumpsensor.real_sensor_read[0]);
  Serial.println(bumpsensor.real_sensor_read[1]);*/
  if (state != COME_BACK) {
    ...
    if (bumpsensor.real_sensor_read[0] == 0 || bumpsensor.real_sensor_read[1] == 0) {
...
void initial_state() {
  ...
  pwm_left = spd_pid_left.update(0.35 + pwm_heading, avg_e1_spd);
...
void follow_the_line() {
  ...
  e_line = linesensor.e_line;
  pwm_heading = heading.update(demand_heading, e_line);
  pwm_left = spd_pid_left.update(0.3 + pwm_heading, avg_e1_spd);
  pwm_right = spd_pid_right.update(0.3 - pwm_heading, avg_e0_spd);
  motors.setMotorPower(pwm_left / 0.018, pwm_right / 0.018);
}

void rotate_180degree() {
...
void come_back_1() {
  while (d == true) {
    ...
    X1 = ((count_e0 / 2) + (count_e1 / 2))*0.28;
    Y1 = kinematics_origin.X_M;
    kinematics_origin.kinematics_cal();
    demand_theta = 180;
    current_theta = kinematics_origin.R_M;
    while (kinematics_origin.R_M < 1.57) {
      if (h == true) {
        spd_pid_left.reset();
        spd_pid_right.reset();
        rotate.reset();
    ...
    bumpsensor.readBumpSensor();
    while (bumpsensor.real_sensor_read[0] == 1 || bumpsensor.real_sensor_read[1] == 1) {
      ...
      calculate_speed();
        //pwm_heading = rotate.update(demand_theta/2463.7194,current_theta/2463.7194);
        pwm_heading = heading.update(0, 0);
        pwm_left = spd_pid_left.update(+0.36 + pwm_heading, avg_e1_spd);
        pwm_right = spd_pid_right.update(0.36 - pwm_heading, avg_e0_spd);
        motors.setMotorPower(pwm_left / 0.018, pwm_right / 0.018);
      bumpsensor.readBumpSensor();
    }
    X2 = ((count_e0 / 2) + (count_e1 / 2))*0.28;
    kinematics_origin.kinematics_cal();
    demand_theta = 180;
    current_theta = kinematics_origin.R_M;
    ...
    X3 = ((count_e0 / 2) + (count_e1 / 2))*0.28;
    kinematics_origin.kinematics_cal();
    demand_theta = 180;
    current_theta = kinematics_origin.R_M;
    while (kinematics_origin.R_M < 4.71) {
      if (f == true) {
        spd_pid_left.reset();
        spd_pid_right.reset();
        rotate.reset();
